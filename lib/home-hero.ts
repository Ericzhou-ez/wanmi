...
export const homeHero = {
  ...
  styles: {
    ...
    shell:
      "relative h-[80vh] overflow-hidden rounded-none bg-neutral-950 lg:h-auto lg:min-h-[50vh]",
    frame:
      "relative h-full w-full lg:mx-auto lg:h-auto lg:max-w-(--breakpoint-2xl) lg:aspect-[16/7]",
    image: "object-cover",
    gradient:
    ...
    title:
      "mt-3 font-(family-name:--font-playfair) text-4xl leading-[1.05] font-semibold sm:text-5xl lg:text-6xl",
...
export function getHomeHeroHandles() {
  return Array.from(
    new Set(
      homeHero.slides.flatMap((slide) => slide.hotspots.map((h) => h.handle)),
    ),
  );
}
