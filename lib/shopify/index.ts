...
export async function shopifyFetch<T>({
   ...
   try {
      ...
      if (!result.ok) {
         const hint =
            result.status === 401
               ...
               : "";
         throw new Error(`Shopify HTTP ${result.status} ${result.statusText}${hint}`, {
            cause: body,
         });
      }
      ...
      const wrapped = new Error(
         `Shopify request failed: ${unknownErr.message}`,
         { cause: unknownErr },
      );
...
export async function getProduct(handle: string): Promise<Product | undefined> {
   ...
   if (!endpoint) {
      console.log(
         `Skipping getProduct for '${handle}' - Shopify not configured`,
      );
      return undefined;
   }
